...
// A값과 B값이 있다고 하자. A값은 coord 배열의 첫 번 값이고 B값은 coord 배열의 두 번째 값임
...
int compare(const void* A, const void* B) {
    ...
    // return 1 : 첫 번 요소과 두 번째 요소보다 큼을 의미함. 따라서 qsort는 두 요소의 순서를 바꾸지 않음
    ...
    // 만약 A의 x좌표가 A+1보다 클  순서를 바꾸지 않음
    else if (coordA->x == coordB->x) {
        if (coordA->y > coordB->y) {
            return 1;
        }
        // 만약 A의 x좌표와 A+1의 x좌표가 같을 
